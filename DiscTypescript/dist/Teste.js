...
function main() {
    ...
    for (let i = 0; i < quantidadePastas; i++) {
        ...
        if (quantidadeSubpastas > 0) {
            ...
            const quantidadeArquivos = readlineSync.questionInt(`Digite a quantidade de arquivos que deseja adicionar a pasta '${novaPasta.nome}': `);
...
function criarSubpastas(readlineSync, pastaPai, quantidadeSubpastas) {
    for (let i = 0; i < quantidadeSubpastas; i++) {
        ...
        const quantidadeArquivos = readlineSync.questionInt('Digite a quantidade de arquivos que deseja adicionar a subpasta: ');
        criarArquivos(readlineSync, novaSubpasta, quantidadeArquivos);
    }
}
function criarArquivos(readlineSync, pasta, quantidadeArquivos) {
